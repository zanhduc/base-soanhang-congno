    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-slate-900/60 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div>
            <div className="text-xs uppercase tracking-widest mb-1 text-slate-500">Chạy hàm</div>
        ...
        <div className="flex flex-col gap-1.5 mb-4">
          <label className="text-xs font-medium text-slate-600">
            Tham số <span className="text-slate-400 font-normal">(JSON, ngăn cách dấu phẩy)</span>
          </label>
          <input
            value={args}
...
export default function DashboardPage({ user, onLogout }) {
  ...
  const [quickTests, setQuickTests] = useState([
    { label: "helloServer", fn: "helloServer", args: "", desc: "Kiểm tra server" },
    { label: "getUserInfo", fn: "getUserInfo", args: `"${user?.email}"`, desc: "Lấy thông tin người dùng" },
  ])
      ...
      .then((res) => {
        if (res?.success && Array.isArray(res.data)) {
          const loginTests = res.data.map((acc) => ({
            ...
            desc: `Đăng nhập bằng ${acc.name || acc.role}`,
          }))

          setQuickTests((prev) => [
            prev[0],
            ...loginTests,
            { label: "login (sai mật khẩu)", fn: "login", args: `"admin@demo.com", "wrongpass"`, desc: "Thử đăng nhập thất bại" },
            prev[1],
          ])
        }
      })
      ...
      <div className="flex min-h-screen font-sans bg-slate-100 text-slate-800">
        <aside className="flex flex-col px-3 py-5 sticky top-0 h-screen overflow-y-auto w-[220px] bg-slate-50 border-r border-slate-200 shrink-0">
          <div className="flex items-center gap-2 px-2 py-1.5 mb-5">
            <span className="drop-shadow-md">› ï¸</span>
          ...
          <div className="flex-1">
            ...
            {quickTests.map((qt, i) => (
              ...
              >
                <span className="w-4 h-4 rounded flex items-center justify-center shrink-0 text-xs bg-rose-100 text-rose-700">¡</span>
        ...
        <main className="flex-1 flex flex-col gap-6 p-8 overflow-y-auto max-w-[900px] mx-auto w-full">
          <div>
            ...
            <p className="text-sm mt-1 text-slate-500">Chạy trực tiếp các hàm GAS và xem kết quả theo thời gian thực</p>
          </div>
          ...
          <div className="p-5 rounded-xl bg-white border border-slate-200 shadow-sm">
            <div className="flex gap-4 items-end flex-wrap">
              ...
              <div className="flex flex-col gap-1.5 flex-[3] min-w-[180px]">
                <label className="text-xs font-medium text-slate-600">Tham số</label>
                <input
                  value={argsText}
          ...
          <div className="flex-1 flex flex-col rounded-xl overflow-hidden bg-white border border-slate-200 shadow-sm min-h-[300px]">
            <div className="flex items-center justify-between px-4 py-3 bg-slate-50 border-b border-slate-200">
              <span className="text-xs font-medium text-slate-500">{logs.length} lượt gọi</span>
              {logs.length > 0 && (
                <button
            ...
            {logs.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center gap-3 p-14 text-slate-400">
                ...
                <p className="text-sm">Chọn một API ở thanh bên hoặc nhập tên hàm để bắt đầu</p>
              </div>
            ) : (
